...
import {
  ...
  runFunctionalResidueToolStream,
  type QuickToolsMeta,
...
const DEFAULT_META: QuickToolsMeta = {
  ...
  residue_mapping_function: ["Activity Site"],
  protein_properties_function: [],
  llm_models: ["DeepSeek", "ChatGPT", "Gemini"]
};

export function FunctionalResiduePage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      setMeta(loaded);
      if (loaded.residue_mapping_function.length > 0) setTask(loaded.residue_mapping_function[0]);
