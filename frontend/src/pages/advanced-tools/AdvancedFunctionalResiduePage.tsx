...
import {
  ...
  runAdvancedFunctionalResidueStream,
...
export function AdvancedFunctionalResiduePage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      setMeta(loaded);
      const firstTask = Object.keys(loaded.residue_mapping_function)[0];
      if (firstTask) setTask(firstTask);
      if (loaded.residue_model_mapping_function.length > 0) setModelName(loaded.residue_model_mapping_function[0]);
