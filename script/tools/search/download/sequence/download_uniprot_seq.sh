...
python src/tools/search/download/sequence/download_uniprot_seq.py \
...
for dir in "$base_dir"/IPR*; do
    if [ -d "$dir" ]; then
        ...
        python src/tools/search/download/sequence/download_uniprot_seq.py -f $base_dir/$inter_id/failed.txt -o data/motif_unfold
    fi
done
