...
# Run from project root: ./script/tools/database/test_uniprot.sh

OUT_DIR="example/database/uniprot"
mkdir -p "$OUT_DIR"

echo "=== uniprot_operations ==="
python src/tools/database/uniprot/uniprot_operations.py --test 2>&1 | tee "${OUT_DIR}/test_uniprot_operations.log"
