# VenusFactory2 下载模块使用指南

下载模块帮助您从主要生物数据库快速检索蛋白质数据。无需访问多个数据库网站并逐个下载文件，使用此工具可直接批量下载序列、结构和元数据。

**常见用途**：
- 从 UniProt 下载一系列 ID 的蛋白质序列
- 从 PDB 或 AlphaFold 获取蛋白质的 3D 结构
- 从 InterPro 检索结构域信息
- 一次性批量下载数百个文件，而不是手动点击
