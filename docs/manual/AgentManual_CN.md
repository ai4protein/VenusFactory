...
**Agent** 是一个由 AI 驱动的对话界面，让您可以用自然语言分析蛋白质。不需要点击多个工具和标签页，只需用简单的中文或英文描述您想做什么，Agent 就会自动选择合适的模型、运行分析并为您整合结果。

---

## 1. Agent 能帮你做什么？

Agent 旨在帮助您快速完成蛋白质工程任务，无需学习复杂的工具配置：

- **用自然语言提问**: 例如"这个蛋白的溶解性如何？"或"找出能提高稳定性的突变"
- **轻松上传数据**: 粘贴序列、上传 FASTA/PDB 文件，或提供 UniProt ID
- **获得综合答案**: Agent 自动组合多个模型的预测结果，并用简单的语言解释
- **对话式工作**: 提出后续问题时无需重新上传数据 - Agent 会记住对话上下文
- **节省时间**: 不用分别运行多个分析，Agent 在一次对话中完成所有任务

---

## 2. 界面概览

Agent 界面是一个简洁的聊天环境，专为流畅的对话式交互而设计。
...
| :--- | :--- | :--- |
| **Function/Structural Localization (功能/结构定位)** | “这个蛋白质是位于细胞核的可能性有多大？” “这个锌指蛋白上的 **DNA binding site (DNA 结合位点)**在哪里？” | **Protein Function (蛋白质功能预测)** (Localization, Sorting Signal)；**Functional Residue  (功能残基预测)** (Binding Site) |
| **Rational Design & Optimization (理性设计与优化)** | “找出 5 个同时增加**活性**和**稳定性**的最佳突变。” “这个序列的**不稳定性指数** $>40$，推荐 3 个稳定化突变。” | **Directed Evolution (智能定向进化)** (Activity, Stability)；**Physical & Chemical Properties (理化性质)** |
...
- **注意事项 (重要)**: **Agent 不保留永久的用户数据或会话记忆。** 当您关闭浏览器或开始一个新的聊天时，之前的对话记录和上下文将被清除。**请务必自行保存**任何重要的对话记录和预测结果。