## Agent: AI 助理与自动化工作流引擎

**Agent** 是一个强大的引擎，它建立在**多智能体架构**之上，将传统的预测界面转变为**对话式工作流**。通过使用 **Planner (规划器)**、**Workers (执行器)** 和 **Analysis modules (分析模块)**，它帮助用户通过自然语言管理、执行和解释复杂的蛋白质工程任务。

---

## 1. 简介与核心功能

**Agent** 的核心价值在于自动化实验设计和支持复杂、跨模块的工作流。
