...
预测模块让您能够将训练好的模型应用于新的蛋白质序列来预测其性质。在训练好模型（或使用预训练模型）后，您可以快速预测单个序列的性质，或在批量模式下处理数百个序列。

**两种主要用途**：
- **单序列预测**: 快速获得一个蛋白质的结果 - 适合快速检查或交互式探索
- **批量预测**: 上传包含多个序列的 CSV 文件，一次性获得所有序列的预测 - 非常适合筛选大型蛋白质库
