...
try:
    ...
    if _dir.name == "foldseek" and str(_dir.parents[3]) not in sys.path:
        sys.path.insert(0, str(_dir.parents[3]))
    from src.tools.database.foldseek.foldseek_submit import (
        submit_foldseek_job as _submit_job,
        query_foldseek_status as _query_status,
        wait_foldseek_complete as _wait_complete,
    )
    from src.tools.database.foldseek.download_foldseek_m8 import (
        download_foldseek_m8 as _download_m8,
        prepare_foldseek_sequences as _prepare_sequences,
    )
