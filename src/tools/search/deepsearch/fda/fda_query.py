...
Run: python -m src.tools.search.deepsearch.fda.fda_query
"""
...
def _default_fda_cache_dir() -> str:
    """Cache dir under TEMP_OUTPUTS_DIR so it aligns with other intermediate files."""
    load_dotenv()
    base = os.getenv("TEMP_OUTPUTS_DIR", "temp_outputs")
    return os.path.join(base, "fda_cache")
...
class FDACache:
    ...
    def __init__(self, cache_dir: Optional[str] = None, ttl: int = 3600):
        if cache_dir is None:
            cache_dir = _default_fda_cache_dir()
...
class FDAQuery:
    ...
    def __init__(self, api_key: Optional[str] = None, use_cache: bool = True,
        ...
        Args:
            api_key: FDA API key (optional). If None, uses FDA_API_KEY from env or .env.
...
def main():
    """Run minimal openFDA examples (drug events, count, label)."""
    # FDAQuery() loads .env and uses FDA_API_KEY from env or .env
    fda = FDAQuery()
    if not fda.api_key:
        print("Tip: set FDA_API_KEY in .env or env for higher rate limits.\n")

    print("openFDA query examples")
    print("=" * 50)

    print("\n1. Drug adverse events (aspirin, limit=10)...")
    ...
    print("\n3. Drug label (Lipitor)...")
    label = fda.query_drug_label("Lipitor", brand=True)
    if "results" in label and label["results"]:
        r = label["results"][0]
        if "indications_and_usage" in r:
            print("  Indications:", (r["indications_and_usage"][0][:180] + "..."))
        else:
            print("  Label retrieved (no indications_and_usage)")
    else:
        print("  No label result")
    print("\nDone.")
