...
# Allow running as script by path
...
try:
    ...
    try:
        from src.tools.search.database.brenda.brenda_queries import (
            search_enzymes_by_product, search_enzymes_by_substrate,
            get_environmental_parameters, compare_across_organisms,
            get_substrate_specificity, get_cofactor_requirements,
            find_thermophilic_homologs, find_ph_stable_variants,
        ...
        search_enzymes_by_product = search_enzymes_by_substrate = None
        get_environmental_parameters = compare_across_organisms = None
        get_substrate_specificity = get_cofactor_requirements = None
        find_thermophilic_homologs = find_ph_stable_variants = search_by_pattern = None
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Enzyme pathway builder: find_pathway_for_product, build_retrosynthetic_tree, ...")
    parser.add_argument("--test", action="store_true", help="Run tests for non-helper functions; output under example/database/brenda")
    parser.add_argument("--target", type=str, default="lactate", help="Target product for pathway")
    parser.add_argument("--ec", type=str, default="1.1.1.1", help="EC number for substitutions")
    ...
    if args.test:
        ...
        try:
            if not os.environ.get("BRENDA_EMAIL") or not os.environ.get("BRENDA_PASSWORD"):
                ...
                print("  1. identify_metabolite(...)")
                meta = identify_metabolite(args.target)
                print(f"     -> {meta}")
                ...
                tree = build_retrosynthetic_tree(args.target, depth=2)
                print(f"     nodes: {tree['total_nodes']}, edges: {tree['total_edges']}")
    ...
    pathway = find_pathway_for_product(args.target, max_steps=3)
    generate_pathway_report(pathway, filename=os.path.join(out_dir, "pathway_report.txt"))
    print(f"Report under {out_dir}")