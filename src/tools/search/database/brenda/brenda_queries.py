...
try:
    ...
    try:
        from src.tools.search.database.brenda.brenda_client import get_km_values, get_reactions, call_brenda
        BRENDA_CLIENT_AVAILABLE = True
    except ImportError:
        get_km_values = get_reactions = call_brenda = None
...
def extract_substrate_products(entry: str) -> Tuple[List[str], List[str]]:
    """Extract substrates (reactants) and products from a BRENDA reaction entry."""
    parsed = parse_reaction_entry(entry)
    return (parsed.get("reactants", []), parsed.get("products", []))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="BRENDA query utilities.")
    parser.add_argument("--test", action="store_true", help="Run tests for non-helper functions; output under example/database/brenda")
    parser.add_argument("--ec", type=str, default="1.1.1.1", help="EC number")
    parser.add_argument("--substrate", type=str, default="glucose", help="Substrate name for search")
    ...
    if args.test:
        ...
        try:
            if not os.environ.get("BRENDA_EMAIL") or not os.environ.get("BRENDA_PASSWORD"):
                print("  Skip: set BRENDA_EMAIL and BRENDA_PASSWORD to run API tests")
            else:
    ...
    try:
        if args.substrate:
            enzymes = search_enzymes_by_substrate(args.substrate, limit=args.limit)
            print(json.dumps(enzymes, indent=2, default=str))
    except Exception as e:
        print(f"Error: {e}")
        exit(1)