...
def download_pdb_by_id(pdb_id: str, out_dir: str, file_type: str = "pdb", unzip: bool = True):
    """Download a single PDB/CIF file by ID. Returns path to the file (unzipped if unzip=True), or None on failure."""
    os.makedirs(out_dir, exist_ok=True)
    pdb_id = pdb_id.upper()
    file_name = f"{pdb_id}.{download_type_dict[file_type]}"
    msg = download_and_unzip(file_name, out_dir, unzip)
    if "failed" in msg:
        return None
    out_path = os.path.join(out_dir, file_name[:-3] if unzip and file_name.endswith(".gz") else file_name)
