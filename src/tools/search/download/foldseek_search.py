"""
FoldSeek structure search: submit PDB to FoldSeek API, download alignments, extract sequences.
CLI: python src/tools/search/download/foldseek_search.py --pdb_file <path> [--output_dir <dir>] [--protect_start N] [--protect_end N]
"""
import os
import io
import time
...
def download_foldseek_m8(pdb_file_path: str, output_dir: Union[Path, str]) -> List[str]:
...
def get_foldseek_sequences(
    ...
    """
    Run FoldSeek search and extract sequences covering the protected region.
    If output_dir is None, uses download/FoldSeek (or caller can pass web get_save_path).
    Returns (path_to_output_fasta, total_sequences).
    """
    if output_dir is None:
        output_dir = Path("download/FoldSeek")
    output_dir = Path(output_dir)
    download_files = download_foldseek_m8(str(pdb_file_path), output_dir)
