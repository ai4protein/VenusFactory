...
_default_ep = (os.getenv("AGENT_DEFAULT_BACKEND") or "local").strip().lower()
MCP_DEFAULT_BACKEND = _default_ep if _default_ep in ("local", "pjlab") else "local"
...
async def query_uniprot(uniprot_id: str) -> str:
    ...
    try:
        ...
        result = await asyncio.to_thread(uniprot_sequence_download_tool.invoke, {"uniprot_id": validated_input.uniprot_id})
...
async def query_interpro(uniprot_id: str) -> str:
    ...
    try:
        ...
        result = await asyncio.to_thread(interpro_lookup_tool.invoke, {"uniprot_id": validated_input.uniprot_id})
        return format_tool_response(result)
    except ValueError as e:
        return format_tool_response(None, error=e)
    except Exception as e:
...
@mcp.tool()
async def download_pdb_structure(pdb_id: str, output_format: str = "pdb", backend: str = MCP_DEFAULT_BACKEND) -> str:
    """
    ...
    Args:
        ...
        backend (str): local (local path only) or pjlab (upload via SCP).
    Returns:
...
@mcp.tool()
async def download_ncbi_sequence(accession_id: str, output_format: str = "fasta", backend: str = MCP_DEFAULT_BACKEND) -> str:
    """
...
@mcp.tool()
async def download_alphafold_structure(uniprot_id: str, output_format: str = "pdb", backend: str = MCP_DEFAULT_BACKEND) -> str:
    """
...
async def foldseek_search(
    ...
    protect_start: int,
    protect_end: int,
    backend: str = MCP_DEFAULT_BACKEND,
) -> str:
    """
...
@mcp.tool()
async def esmfold_structure_prediction(sequence: str, backend: str = MCP_DEFAULT_BACKEND) -> str:
    """
    ...
    try:
        ...
        result = await asyncio.to_thread(esmfold_structure_prediction_tool.invoke, {
...
@mcp.tool()
async def zero_shot_mutation_sequence_prediction(
    ...
    Args:
        ...
        backend (str): local (local Gradio) or pjlab (pjlab Gradio + SCP upload).
    Returns:
    ...
    try:
        ...
        params = {"model_name": validated_input.model_name, "backend": backend}
        ...
        result = await asyncio.to_thread(zero_shot_mutation_sequence_prediction_tool.invoke, params)
        return format_tool_response(result)
    except ValueError as e:
        return format_tool_response(None, error=e)
    except Exception as e:
...
async def predict_zero_shot_structure(
    ...
    try:
        ...
        result = await asyncio.to_thread(zero_shot_mutation_structure_prediction_tool.invoke, {
...
async def predict_protein_properties(
    ...
    Args:
        ...
        backend (str): local (local Gradio) or pjlab (pjlab Gradio).
    Returns:
        str: Prediction result.
    """
    try:
        ...
        result = await asyncio.to_thread(protein_property_prediction_tool.invoke, params)
        return format_tool_response(result)
    except ValueError as e:
        return format_tool_response(None, error=e)
    except Exception as e:
