...
- **Skill-first for code steps**: For analysis/visualization code workflows, prefer a dedicated `read_skill` step before `python_repl` / `agent_generated_code`. If CB asks for complex code but no relevant skill has been loaded, explicitly report this to CB and request a skill-loading step instead of guessing APIs.
...
- All user-visible narration must stay in the user's language; avoid Chinese-English mixed sentences except for technical identifiers.
- Keep technical identifiers unchanged in English (tool names, parameter keys, dependency tokens, model IDs, file extensions).
