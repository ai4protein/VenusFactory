...
|-----|-----------------|--------|
| esearch | `clinvar_esearch(term, retmax, retstart, retmode, api_key, sort)` or `query_clinvar(term, ...)` | Returns response text (JSON/XML). Use `parse_esearch_ids()` / `parse_esearch_count()` to get idlist and count from JSON. |
| esummary | `clinvar_esummary(id_list, retmode, api_key)` or `get_clinvar_summary(variation_ids, ...)` | id_list = list of Variation IDs (VCV). |
| efetch | `clinvar_efetch(id_list, rettype, retmode, api_key)` or `fetch_clinvar_records(variation_ids, rettype="vcv", retmode="xml", ...)` | rettype: vcv (variation) or rcv. |
| Query term | `build_clinvar_term(gene=..., clinical_significance=..., condition=..., chr=..., variant_name=..., exclude_conflicting=..., raw_term=...)` | Builds term with [gene], [CLNSIG], [disorder], [chr], [variant name], NOT conflicting[RVSTAT]. |

Constants: `CLINVAR_DB`, `EUTILS_BASE`, `FTP_BASE`. FTP: `get_clinvar_ftp_url(key)`, `download_clinvar_ftp(url_or_key, out_dir, filename)`. See SKILL.md for the full function list.
