        ...
        def update_dataset_preview(dataset_type=None, dataset_name=None, custom_dataset=None):
            ...
            if dataset_type == "Use Custom Dataset" and custom_dataset:
                try:
                    ...
                    stats_html = dataset_stats_html(custom_dataset, len(dataset["train"]) if "train" in dataset else 0, len(dataset["validation"]) if "validation" in dataset else 0, len(dataset["test"]) if "test" in dataset else 0)
            ...
            # Use predefined dataset
                ...
                try:
                    ...
                    stats_html = dataset_stats_html(config["dataset"], len(dataset["train"]) if "train" in dataset else 0, len(dataset["validation"]) if "validation" in dataset else 0, len(dataset["test"]) if "test" in dataset else 0)
...
def load_html_template(name: str, **kwargs) -> str:
    ...
    path = os.path.join(os.path.dirname(__file__), "assets", "html_fragments", name)
...
def dataset_stats_html(dataset_name, train_count, val_count, test_count):
    return f"""
    <div style="text-align: center; margin: 20px 0;">
        <table style="width: 100%; border-collapse: collapse; margin: 0 auto;">
            ...
            <tr>
                <td style="padding: 15px; font-size: 14px; border: 1px solid #ddd; text-align: center;">{dataset_name}</td>
                <td style="padding: 15px; font-size: 14px; border: 1px solid #ddd; text-align: center;">{train_count}</td>
                <td style="padding: 15px; font-size: 14px; border: 1px solid #ddd; text-align: center;">{val_count}</td>
                <td style="padding: 15px; font-size: 14px; border: 1px solid #ddd; text-align: center;">{test_count}</td>
            </tr>
        </table>
    </div>
    """

def error_loading_dataset_html(error_message):
    return f"""
    <div>
