...
def generate_mutation_ai_prompt(results_df: pd.DataFrame, model_name: str, function_selection: str) -> str:
    ...
    score_col = next(
        (col for col in results_df.columns if 'score' in col.lower()), 
...
def handle_mutation_prediction(
    ...
    if model_name_override:
        ...
        model_type = "structure" if model_name_override == "ESM-IF1" else "sequence"
    else:
        if file_path.lower().endswith((".fasta", ".fa")):
