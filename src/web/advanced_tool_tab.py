...
def handle_venus_pdb_upload(file):
    """Handle PDB file upload for VenusMine viewer (HTML placeholder; Molecule3D not used with Gradio 5)."""
    if file:
        return load_html_template("pdb_loaded_placeholder.html")
    return load_html_template("venusmine_upload_placeholder.html")
...
def handle_mutation_prediction_advance(
    ...
    expert_analysis = load_html_template("ai_expert_placeholder.html")
    ...
    if enable_ai:
        ...
        api_key = get_api_key(llm_model, user_api_key)
        if not api_key:
...
def handle_protein_function_prediction_advance(
    ...
    if enable_ai:
        ...
        api_key = get_api_key(llm_model, user_api_key)
        if not api_key: 
            ai_summary = f"❌ No API key found for {llm_model}."
        else:
            llm_config = LLMConfig(api_key, llm_model, get_chat_base_url(), LLM_MODELS.get(llm_model, ""))
...
def create_advanced_tool_tab(constant: Dict[str, Any]) -> Dict[str, Any]:
    ...
    with gr.Column():  # Use Column instead of Blocks - nested Blocks causes Component ID error in Gradio 6
        with gr.Tabs():
            with gr.Tab("Directed Evolution"):
                with gr.Row(equal_height=False):
                    with gr.Column(scale=2):
                        with gr.Tabs():
                            with gr.Tab("🧬 Sequence-based Model"):
                                ...
                                with gr.Column(visible=False) as seq_sequence_section:
                            ...
                            with gr.Tab("🏗️ Structure-based Model"):
                                gr.Markdown("## Model Configuration")
                                ...
                                with gr.Column(visible=False) as struct_sequence_section:
                                    gr.Markdown("## Sequence")
                    ...
                    with gr.Column(scale=3):
                        ...
                        with gr.Tabs():
                            ...
                            with gr.Tab("📈 Prediction Heatmap"):
                            ...
                            with gr.Tab("👨‍🔬 AI Expert Analysis"):
                                zero_shot_ai_expert_html = gr.HTML(
                                    value=load_html_template("ai_expert_placeholder.html"),
            ...
            with gr.Tab("Protein Discovery"):
                with gr.Tabs():
                    with gr.Tab("VenusMine"):
                        with gr.Row(equal_height=False):
                            with gr.Column(scale=2):
                                ...
                                # Status indicator
                                venus_status_indicator = gr.HTML(value=load_html_template("venus_status_ready.html"))

                            with gr.Column(scale=5):
                                gr.Markdown("## 📈 Pipeline Results")
                                with gr.Tabs() as venus_result_tabs:
                                    with gr.Tab("🔬 Structure Visualization"):
                                        venus_pdb_viewer = gr.HTML(
                                            value=load_html_template("pdb_viewer_placeholder.html"),
            ...
            with gr.Tab("Protein Function Prediction"):
                with gr.Row(equal_height=False):
                    with gr.Column(scale=2):
                        gr.Markdown("## Model Configuration")
                        ...
                        with gr.Column(visible=False) as function_sequence_section:
                            gr.Markdown("## Sequence")
                            function_protein_display = gr.Textbox(label="Uploaded Protein Sequence", interactive=False, lines=3, max_lines=7)
            ...
            with gr.Tab("Functional Residue "):
                with gr.Row(equal_height=False):
                    with gr.Column(scale=2):
                        gr.Markdown("## Model Configuration")
                        ...
                        with gr.Column(visible=False) as adv_residue_function_sequence_section:
                            gr.Markdown("## Sequence")
                    ...
                    with gr.Column(scale=3):
                        ...
                        adv_residue_function_result_state = gr.State()
        ...
        llm_dd_stru_zshot.change(
            fn=on_llm_change,
            inputs=llm_dd_stru_zshot,
            outputs=[api_key_in_stru_zshot, ai_status_stru_zshot]
        )
        llm_dd_seq_zshot.change(
            fn=on_llm_change,
            inputs=llm_dd_seq_zshot,
            outputs=[api_key_in_seq_zshot, ai_status_seq_zshot]
        )
        llm_dd_seq_func.change(
            fn=on_llm_change,
            inputs=llm_dd_seq_func,
            outputs=[api_key_in_seq_func, ai_status_seq_func]
        )
        llm_dd_residue_function.change(
            fn=on_llm_change,
            inputs=llm_dd_residue_function,
        ...
        seq_file_upload.upload(
            ...
            outputs=[seq_protein_display, seq_sequence_selector, seq_sequence_state, seq_selected_sequence_state, seq_original_file_path_state, seq_current_file_state, seq_sequence_section]
        )
        ...
        seq_paste_clear_btn.click(
            ...
            outputs=[seq_paste_content_input, seq_protein_display, seq_sequence_selector, seq_sequence_state, seq_selected_sequence_state, seq_original_file_path_state, seq_sequence_section]
        )

        seq_paste_content_btn.click(
            fn=lambda c: wrap_paste_show_section(handle_paste_fasta_detect(c)),
            inputs=seq_paste_content_input,
            outputs=[seq_protein_display, seq_sequence_selector, seq_sequence_state, seq_selected_sequence_state, seq_original_file_path_state, seq_original_paste_content_state, seq_file_upload, seq_sequence_section]
        )
        ...
        struct_file_upload.upload(
            ...
            outputs=[struct_protein_display, struct_chain_selector, struct_chains_state, struct_selected_chain_state, struct_original_file_path_state, struct_current_file_state, struct_sequence_section]
        )
        ...
        struct_paste_clear_btn.click(
            fn=lambda: wrap_clear_hide_section(clear_paste_content_pdb()),
            outputs=[struct_paste_content_input, struct_protein_display, struct_chain_selector, struct_chains_state, struct_selected_chain_state, struct_original_file_path_state, struct_sequence_section]
        )
        
        struct_paste_content_btn.click(
            fn=lambda c: wrap_paste_show_section(handle_paste_pdb_detect(c)),
            inputs=struct_paste_content_input,
            outputs=[struct_protein_display, struct_chain_selector, struct_chains_state, struct_selected_chain_state, struct_original_file_path_state, struct_original_paste_content_state, struct_sequence_section]
        )
        ...
        function_fasta_upload.upload(
            ...
            outputs=[function_protein_display, function_protein_selector, function_sequence_state, function_selected_sequence_state, function_original_file_path_state, function_current_file_state, function_sequence_section]
        )
        ...
        function_paste_clear_btn.click(
            fn=lambda: wrap_clear_hide_section(clear_paste_content_fasta()),
            outputs=[function_paste_content_input, function_protein_display, function_protein_selector, function_sequence_state, function_selected_sequence_state, function_original_file_path_state, function_sequence_section]
        )

        function_paste_content_btn.click(
            fn=lambda c: wrap_paste_show_section(handle_paste_fasta_detect(c)),
            inputs=function_paste_content_input,
            outputs=[function_protein_display, function_protein_selector, function_sequence_state, function_selected_sequence_state, function_original_file_path_state, function_original_paste_content_state, function_fasta_upload, function_sequence_section]
        )
        ...
        adv_residue_function_fasta_upload.upload(
            ...
            outputs=[adv_residue_function_protein_display, adv_residue_function_selector, adv_residue_function_sequence_state, adv_residue_function_selected_sequence_state, adv_residue_function_original_file_path_state, adv_residue_function_current_file_state, adv_residue_function_sequence_section]
        )
        ...
        adv_residue_function_paste_clear_btn.click(
            fn=lambda: wrap_clear_hide_section(clear_paste_content_fasta()),
            outputs=[adv_residue_function_paste_content_input, adv_residue_function_protein_display, adv_residue_function_selector, adv_residue_function_sequence_state, adv_residue_function_selected_sequence_state, adv_residue_function_original_file_path_state, adv_residue_function_sequence_section]
        )
        adv_residue_function_paste_content_btn.click(
            fn=lambda c: wrap_paste_show_section(handle_paste_fasta_detect(c)),
            inputs=adv_residue_function_paste_content_input,
            outputs=[adv_residue_function_protein_display, adv_residue_function_selector, adv_residue_function_sequence_state, adv_residue_function_selected_sequence_state, adv_residue_function_original_file_path_state, adv_residue_function_original_paste_content_state, adv_residue_function_fasta_upload, adv_residue_function_sequence_section]
        )
        ...
        adv_residue_function_predict_btn.click(
            ...
            inputs=[adv_residue_function_task_dd, adv_residue_function_fasta_upload, enable_ai_residue_function, llm_dd_residue_function, api_key_in_residue_function, adv_residue_function_model_dd],
            outputs=[adv_residue_function_status_textbox, adv_residue_function_results_df, adv_residue_function_plot_out, adv_residue_function_download_btn, adv_residue_function_ai_expert_html, adv_residue_function_result_state]
        )
        ...
        seq_predict_btn.click(
            ...
            inputs=[seq_function_dd, seq_file_upload, enable_ai_zshot_seq, llm_dd_seq_zshot, api_key_in_seq_zshot, seq_model_dd],
        ...
        struct_predict_btn.click(
            fn=handle_mutation_prediction_advance, 
            inputs=[struct_function_dd, struct_file_upload, enable_ai_zshot_stru, llm_dd_stru_zshot, api_key_in_stru_zshot, struct_model_dd], 
            outputs=[zero_shot_status_box, zero_shot_plot_out, zero_shot_df_out, zero_shot_download_btn, zero_shot_download_path_state, zero_shot_view_controls, zero_shot_full_data_state, zero_shot_ai_expert_html],
            show_progress=True
        )
        ...
        adv_func_predict_btn.click(
            ...
            inputs=[adv_func_task_dd, function_fasta_upload, enable_ai_func, llm_dd_seq_func, api_key_in_seq_func, adv_func_model_dd, adv_func_dataset_cbg],
        ...
        function_protein_chat_btn.click(
            ...
            inputs=[adv_func_task_dd, function_fasta_upload, adv_func_model_dd, adv_func_dataset_cbg_chat, enable_ai_func, llm_dd_seq_func, api_key_in_seq_func],
