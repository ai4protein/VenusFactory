...
def scan_folders_under(root: str, max_depth: int = 5) -> list:
    """Scan root for subdirectories. Returns list of (display_label, full_path) for selection. Uses normalized paths."""
    ...
    for path in sorted(root_path.rglob("*")):
        ...
        if not rel.parts or len(rel.parts) > max_depth:
            continue
        # Use path as string relative to cwd for consistency
        path_str = str(path)
...
def scan_models_in_folder(folder_path: str) -> list:
    """Scan folder for .pt model files (strict: only direct and nested .pt, no LoRA aux). Returns list of (display_label, full_path)."""
...
def create_eval_tab(constant):
    ...
    def evaluate_model(model_path, is_custom_dataset, dataset_defined, dateset_custom,
                       plm_model, eval_method, problem_type, num_labels, metrics,
                       pooling_method, sequence_column_name, label_column_name,
                       eval_structure_seq, batch_mode, batch_size, batch_token, eval_pdb_dir):
        ...
        try:
            ...
            if eval_pdb_dir and str(eval_pdb_dir).strip():
                args_dict["pdb_dir"] = str(eval_pdb_dir).strip()
    ...
    # Layout: Model and Dataset Configuration (one row) -> Hyperparameter Settings (accordion) -> three buttons
    _folder_choices = [(base, base) for base in ("ckpt",) if os.path.isdir(base)] + scan_folders_under("ckpt")
    gr.Markdown("💡 *Evaluate model metrics on labeled datasets.*")
    gr.Markdown("## Model and Dataset Configuration")
    with gr.Group(elem_classes=["eval-config-section"]):
        with gr.Row():
            eval_model_folder = gr.Dropdown(
            ...
            with gr.Column(scale=1, min_width=120, elem_classes="preview-button-container"):
                preview_dataset_btn = gr.Button(
                    "Preview Dataset",
                    variant="primary",
                    size="lg",
                    elem_classes=["preview-button"]
                )
        with gr.Row(visible=False) as eval_pdb_dir_row:
            eval_pdb_dir = gr.Textbox(
                label="PDB Directory",
    ...
    # Hidden: auto-filled from model config and dataset config
    with gr.Row(visible=False):
        eval_plm_model = gr.Dropdown(choices=list(plm_models.keys()), value=list(plm_models.keys())[0] if plm_models else None)
        eval_method = gr.Dropdown(choices=["full", "freeze", "ses-adapter", "plm-lora", "plm-qlora", "plm-adalora", "plm-dora", "plm-ia3"], value="freeze")
        eval_pooling_method = gr.Dropdown(choices=["mean", "attention1d", "light_attention"], value="mean")
        ...
        sequence_column_name = gr.Textbox(value="aa_seq")
        label_column_name = gr.Textbox(value="label")

    with gr.Row(visible=False) as eval_structure_seq_row:
        eval_structure_seq = gr.CheckboxGroup(
            choices=["foldseek_seq", "ss8_seq"],
            value=[],
            label="Structure Sequences (required for ses-adapter)"
        )

    def on_model_folder_change(folder_path):
        """Strictly list .pt files in selected folder. No .pt = empty dropdown."""
        if not folder_path:
            return [gr.update(choices=[], value=None)] + on_model_selected(None)
        choices = scan_models_in_folder(folder_path)
        if not choices:
            return [gr.update(choices=[], value=None)] + on_model_selected(None)
        first_path = choices[0][1]
        cfg_upds = on_model_selected(first_path)
        return [gr.update(choices=choices, value=first_path)] + cfg_upds

    def on_model_selected(model_path):
        """Load model config and return updates for hidden components."""
        cfg = load_model_config(model_path) if model_path else {}
        if not cfg:
            return [gr.update() for _ in range(9)] + [gr.update(visible=False), gr.update(visible=False)]
        ...
        eval_method_val = cfg.get("training_method", "freeze")
        if eval_method_val not in ["full", "freeze", "ses-adapter", "plm-lora", "plm-qlora", "plm-adalora", "plm-dora", "plm-ia3"]:
            eval_method_val = "freeze"
        metrics_val = cfg.get("metrics", ["accuracy", "mcc", "f1"])
        if isinstance(metrics_val, str):
            metrics_val = [m.strip() for m in metrics_val.split(",")]
        struct_seq = cfg.get("structure_seq", [])
        if not isinstance(struct_seq, list):
            struct_seq = [struct_seq] if struct_seq else []
        struct_seq = [s for s in struct_seq if s in ("foldseek_seq", "ss8_seq")]
        show_pdb = (eval_method_val == "ses-adapter") or bool(plm_key and (str(plm_key).startswith("ProSST") or str(plm_key).startswith("SaProt") or str(plm_key).startswith("ProtSSN")))
        show_struct = eval_method_val == "ses-adapter"
        return [
            gr.update(value=plm_key),
            gr.update(value=eval_method_val),
            ...
            gr.update(value=metrics_val),
            gr.update(value=cfg.get("sequence_column_name", "aa_seq")),
            gr.update(value=cfg.get("label_column_name", "label")),
            gr.update(value=struct_seq),
            gr.update(visible=show_pdb),
            gr.update(visible=show_struct),
        ]

    eval_model_folder.change(
        fn=on_model_folder_change,
        inputs=[eval_model_folder],
        outputs=[eval_model_dropdown, eval_plm_model, eval_method, eval_pooling_method, problem_type, num_labels, metrics, sequence_column_name, label_column_name, eval_structure_seq, eval_pdb_dir_row, eval_structure_seq_row]
    )
    eval_model_dropdown.change(
        fn=on_model_selected,
        inputs=[eval_model_dropdown],
        outputs=[eval_plm_model, eval_method, eval_pooling_method, problem_type, num_labels, metrics, sequence_column_name, label_column_name, eval_structure_seq, eval_pdb_dir_row, eval_structure_seq_row]
    )

    def _eval_plm_needs_structure(plm: str) -> bool:
        return bool(plm and (str(plm).startswith("ProSST") or str(plm).startswith("SaProt") or str(plm).startswith("ProtSSN")))

    def update_eval_visibility(method, plm):
        show_pdb = (method == "ses-adapter") or _eval_plm_needs_structure(plm)
        show_structure_seq = method == "ses-adapter"
        return gr.update(visible=show_pdb), gr.update(visible=show_structure_seq)

    eval_method.change(fn=update_eval_visibility, inputs=[eval_method, eval_plm_model], outputs=[eval_pdb_dir_row, eval_structure_seq_row])
    eval_plm_model.change(fn=update_eval_visibility, inputs=[eval_method, eval_plm_model], outputs=[eval_pdb_dir_row, eval_structure_seq_row])
    ...
    with gr.Row():
        preview_command_btn = gr.Button("Preview Command", elem_classes=["preview-command-btn"])
    ...
    def update_eval_tab_dataset_preview_UI(dataset_type=None, dataset_name=None, custom_dataset=None):
        ...
        if dataset_type == "Custom" and custom_dataset:
            try:
                ...
                if len(samples) == 0:
                    return gr.update(value=stats_html), gr.update(value=[["No data available", "-", "-"]], headers=["Name", "Sequence", "Label"])
                ...
                return gr.update(value=stats_html), gr.update(value=sample_data, headers=available_fields)
            except Exception as e:
                error_html = load_html_template("error_loading_dataset.html", error_message=str(e))
                return gr.update(value=error_html), gr.update(value=[["Error", str(e), "-"]], headers=["Error", "Message", "Status"])
        ...
        return gr.update(value=""), gr.update(value=[["No dataset selected", "-", "-"]], headers=["Status", "Message", "Action"])

    
    # Preview Dataset button click event
    ...
    def handle_eval_dataset_defined_change(x):
        ...
        """
        return update_eval_tab_dataset_settings_UI("Pre-defined", x)
    ...
    def update_eval_tab_batch_inputs_UI(mode):
        return (
            gr.update(visible=mode == "Batch Size Mode"),
            gr.update(visible=mode == "Batch Token Mode")
        )
    ...
    def handle_eval_tab_command_preview(plm_model, model_path, eval_method, is_custom_dataset, dataset_defined, 
                        dataset_custom, problem_type, num_labels, metrics, 
                        batch_mode, batch_size, batch_token, eval_structure_seq, eval_pooling_method, sequence_column_name, label_column_name, eval_pdb_dir):
