        ...
        if file_paths:
            ...
            for fp in file_paths:
                ...
                if file_ext in ['.fasta', '.fa']:
                    file_context += f"- {file_name}: FASTA sequence file at path '{fp}'\n"
                elif file_ext == '.pdb':
                    file_context += f"- {file_name}: PDB structure file at path '{fp}'\n"
                elif file_ext == '.csv':
                    file_context += f"- {file_name}: Data file at path '{fp}'\n"
                else:
                ...
                try:
                    ...
                    for key, value in tool_input.items():
                        if isinstance(value, str) and value.startswith("dependency:"):
                            ...
                            parts_1 = ""
                            if "_" in parts[1]:
                                parts_1 = parts[1].replace('step_','')
                            else:
                                parts_1 = parts[1].replace('step','')
                            dep_step, dep_key = int(parts_1), parts[2]
